...
def analyze_training_results(log_path: str, output_dir: str):
    ...
    # Training loss
    axes[0, 0].plot(
        df["step"], df["train_mean_nll"], marker="o", linewidth=2, markersize=4, color="#2E86AB"
    )
    ...
    # Time per step
    axes[1, 1].plot(
        df["step"], df["time/step"], marker="o", linewidth=2, markersize=4, color="#C73E1D"
    )
    ...
    with open(report_file, "w") as f:
        ...
        f.write(
            f"  Loss reduction:   {df['train_mean_nll'].iloc[0] - df['train_mean_nll'].iloc[-1]:.4f}\n\n"
        )

        f.write("Training Time:\n")
        f.write(
            f"  Total time:       {df['time/total'].sum():.2f} seconds ({df['time/total'].sum() / 60:.2f} minutes)\n"
        )
        ...
        if "epoch" in df.columns:
            ...
            epoch_stats = (
                df.groupby("epoch")["train_mean_nll"].agg(["mean", "min", "max", "count"]).round(4)
            )
